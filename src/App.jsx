  ...
  function renderComment(c) {
    ...
    return (
      <div key={c.id} style={{ padding:"14px 18px 10px", borderBottom:`1px solid ${C.gray100}` }}>
        {/* Single outer flex row: left col spans avatar + line for entire thread */}
        <div style={{ display:"flex", gap:10, alignItems:"stretch" }}>
          {/* Left col: avatar at top, line fills remaining height if replies exist */}
          <div style={{ width:34, flexShrink:0, display:"flex", flexDirection:"column", alignItems:"center" }}>
            ...
            {(replies.length > 0 || rs.open) && (
              <div style={{ width:2, flex:1, background:C.gray200, marginTop:6, borderRadius:1 }}/>
            )}
          </div>

          {/* Right col: comment content + replies stacked */}
          <div style={{ flex:1, minWidth:0 }}>
            {/* Comment content */}
            ...
            {/* Replies threaded below — each with its own avatar + line sub-column */}
            {replies.map((r, ri) => {
              const isLast = ri === replies.length - 1 && !rs.open;
              return (
                <div key={r.id} style={{ display:"flex", gap:10, alignItems:"stretch", paddingTop:12 }}>
                  <div style={{ width:28, flexShrink:0, display:"flex", flexDirection:"column", alignItems:"center" }}>
                    ...
                    {!isLast && <div style={{ width:2, flex:1, background:C.gray200, marginTop:6, borderRadius:1 }}/>}
                  </div>
